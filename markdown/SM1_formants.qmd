...
vowels <- vowels %>% 
  filter(
    !is.na(F1_50) # NA cases should be the same for both formants.
  )
...
Finally, we filter out very extreme F1 values. In cases with extreme F1 values,
F2 will almost certainly be unreliable.
...
f1_extreme_count <- vowels |> 
  filter(
    !high_f1_bandwidth, !F1_outlier
  ) |> 
  nrow()

f2_extreme_count <- vowels |> 
  filter(
    !high_f2_bandwidth, !F2_outlier
  ) |> 
  nrow()
```
...
Let's look at a distribution of ages at recording:
``` {r fig-participant-ages, fig.cap = "Participants by age at recording."}
participants_data %>% 
  ggplot(
    aes(
      x = age
  ...
  geom_histogram(stat="count") +
  labs(title = "Participants by age at recording", x = "Age (months)") +
  theme(
    axis.text.x = element_text(angle = 90)
  )
```
...
f1_lob2_count <- nrow(
  lob2_imputed %>% 
    filter(!high_f1_bandwidth, !F1_outlier, !is.na(F1_lob2))
)
...
## Post-normalisation Data Summary
...
We also regenerate the filtering flow chart to include the post-normalisation
filtering of participant collection points.

```{r}
#| label: fig-filtering-flow-2
#| fig.cap: "Data filtering flow chart, after normalisation."
#| fig.height: 10
# Flow chart generated using the DiagrammeR package.
flow_chart <- mermaid(
  diagram = glue("
  ...
  I --> K(F1: extremes: {f1_extreme_count} tokens)
  J --> L(F2: extremes: {f2_extreme_count} tokens)
  K --> O(F1: low-token speakers: {f1_lob2_count} tokens)
  L --> P(F2: low-token speakers: {f2_lob2_count} tokens)
  "),
  width = 800
)
...
And now we generate the counts for the summary tables in the paper.

Participant summary:
```{r}
participants_collect <- lob2_imputed %>%
  group_by(participant) %>% 
  summarise(
    collection_points = n_distinct(collect)
  ) %>% 
...
```{r}
participants_data <- lob2_imputed %>% 
  group_by(participant) %>%
  summarise(
    gender = first(gender),
    ethnicity = first(ethnicity),
...
Total number of F2 tokens is `r sum(f1_counts$n)`.

```{r}
# F2 counts.
f2_counts <- lob2_imputed |> 
  filter(!high_f2_bandwidth, !F2_outlier, !is.na(F2_lob2)) |> 
  count(vowel) |> 
  arrange(vowel)
...
Total number of F2 tokens is `r sum(f2_counts$n)`.

Let's look at the age variables:
```{r}
min_age <- min(participants_data$age)
min_years <- min_age %/% 12
min_months <- min_age %% 12
glue("min age: {min_years};{min_months}")

med_age <- median(participants_data$age)
med_years <- med_age %/% 12
med_months <- med_age %% 12
glue("median age: {med_years};{med_months}")

max_age <- max(participants_data$age)
max_years <- max_age %/% 12
max_months <- max_age %% 12
glue("max age: {max_years};{max_months}")
```

```{r}
min_dob <- min(participants_data$dob)
min_dob

med_dob <- median(participants_data$dob)
med_dob

max_dob <- max(participants_data$dob)
max_dob
```

```{r}
#| label: fig-age-dens-postnorm
#| fig.cap: "Distribution age ages after normalisation."
