...
cat('Tokens from younger adult comparison group: ', nrow(QB1_caregivercount), '\n')
...
cat('Younger adult comparison: ', QB1_caregivercount$n[[1]], '\n')  
...
#| fig.lab: | 
#|   Change over time for KIT F1. Higher 'age_cat_s' values are older speakers.
...
Let's see this for one other vowels discussed in the body text of the article:
[nurse]{.smallcaps}. For both [kit]{.smallcaps} and [nurse]{.smallcaps}, we
mention apparent reversals and discuss their timing. For [nurse]{.smallcap} we
could look at either F1 or F2. Let's look at F1.

```{r}
#| label: fig-qb-age-nurse-f1
#| fig.lab: | 
#|   Change over time for NURSE F1. Higher 'age_cat_s' values are older speakers.
nurse_rel <- estimate_relation(
  QB1_models |> 
    filter(
      vowel == "NURSE",
...
```{r}
# Modify this block to define the 'generations' used to interpret the
# results.
QB1_preds <- QB1_preds %>% 
  mutate(
    generation = case_when(
      between(age_cat, 6.9, 8) & gender == "f" ~ "Oldest",
      between(age_cat, 0.9, 2.1) & gender == "f" ~ "Younger", # only f caregivers
      # between(age_cat, 0.9, 2.1) ~ "Younger", # f and m caregivers
      .default = "Other"
    )
  )
```
...
ref_points <- QB1_preds %>% 
  ...
  mutate(
    generation = case_when(
      ...
      generation == "Younger" ~ "Younger adults (F)",
      .default = generation
    )
  ) %>% 
  crossing(
...
kids_plot <- model_widepreds %>% 
  ...
  #   #nudge_x = 0.2,
  #   #nudge_y = 0.1,
  #   min.segment.length = 100 # i.e. no segments.
  #   ) +
  # Switch to manual label locations
  geom_label(
    aes(
      x = F2,
      y = F1,
    ...
    data = tribble(
      ~vowel, ~gender, ~F1, ~F2,
      "FLEECE", "Female", -1.3, 1.70,
      "KIT", "Female", -0.95, 0.9,
      "DRESS", "Female", -0.05, 1.2,
      "NURSE", "Female", -0.65, -0.25,
      "TRAP", "Female", 0.4, 0.9,
      "FLEECE", "Male", -1.35, 1.7,
      "KIT", "Male", -0.95, 0.9,
      "DRESS", "Male", -0.15, 1.2,
      "NURSE", "Male", -0.55, -0.5,
      "TRAP", "Male", 0.7, 0.5
  ...
  geom_point(aes(shape = generation), size = 4, data = ref_points) +
...
As another side-path, we considered what would happen if we did not restrict
the 'community comparison younger adults' to females. The following is the 
vowel space which results:
```{r}
#| label: fig-vs-all-preschoolers-male
#| fig-cap: |
#|   Model predictions from age 4;2 to 5 with reference points from QuakeBox 
#|   corpus and 'age matched comparison' including male speakers.  
#|   70% and 90% credible intervals are plotted.
ref_points_2 <- QB1_preds %>% 
  mutate(
    generation = case_when(
      between(age_cat, 6.9, 8) & gender == "f" ~ "Oldest",
      between(age_cat, 0.9, 2.1) ~ "Younger",
      .default = "Other"
    )
  ) %>%
  group_by(vowel, generation) %>% 
...
kids_plot_2 <- model_widepreds %>% 
  ...
  geom_point(aes(shape = generation), size = 4, data = ref_points_2) +
  scale_x_reverse(expand = expansion(mult = 0.1)) +
  scale_y_reverse() +
  scale_colour_manual(values = vowel_colours_5) +
  scale_fill_manual(values = vowel_colours_5) +
